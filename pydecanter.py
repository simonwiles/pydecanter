...
try:
    ...
    print('=================================')
    print('requires W3C fork of tidy -- '
...
class PyDecanter(object):
    ...
    def __init__(self, args):
        ...
        if 'private' in args:
            self.PRIVATE_FILES = self.PRIVATE_FILES + \
                tuple(re.compile(_) for _ in args.private.split(','))
        ...
        self.app.route('{0}<path:path>'.format(
            self.base_url), method='GET', callback=self.render)
        self.app.route(
        ...
        for ext, mimetype in FORCE_MIMETYPES:
            if path.endswith(ext):
                return bottle.static_file(
                    path, root=self.base_root, mimetype=mimetype)
            ...
            if 'onerror' in img.attrs and re_svgfallback.match(img['onerror']):
                img['onerror'] = re_svgfallback.sub(
                    lambda m: m.group(0).replace(
                        m.group(1), add_cache_tag(
                            m.group(1), self.base_url, self.base_root)
                    ), img['onerror'])
    ...
    def build_static(self, args):
        ...
        for dirpath, dirnames_, filenames in os.walk(self.base_root):
            for filename in filenames:
                path = os.path.relpath(
                    os.path.join(dirpath, filename), self.base_root)
        ...
        for filepath in public_files:
            ...
            if (self.context['compress'] and self.RE_CACHEBUSTABLE.match(filepath) and not
                    any(exc in filepath for exc in self.CACHE_TAG_EXCS)):
            ...
            if filepath.endswith(('.html', '.css', '.js')):
                ...
                shutil.copy2(
                    os.path.join(self.base_root, filepath), output_path)
...
def get_config_from_ini_file(args):
    ...
    if args_dict.get('base_root', None) is None and 'base_root' in ini_args:
        args_dict['base_root'] = ini_file_location / os.path.expanduser(ini_args['base_root'])
    else:
...
def main():
    ...
    parser.add_argument(
        '-i', '--ini-file', action='store', default=None,
    ...
    parser.add_argument(
        ...
        help='port to bind the WSGI server to')

    parser.add_argument(
        '-H', '--host', default=None,
        help='host to bind the WSGI server to')

    parser.add_argument(
        '-f', '--base-root', default=None,
        help='root dir to serve files from')

    parser.add_argument(
        '-r', '--base-url', default=None,
        help='specify a base URL to serve from (e.g. /subfolder)')

    parser.add_argument(
        '-c', '--compress', action='store_true', default=None,
    ...
    args_dict = vars(args)
    args_dict.update({k: v for k, v in DEFAULT_ARGS.items()
